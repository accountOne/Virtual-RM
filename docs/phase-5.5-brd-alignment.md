...
| **Speech-to-Text** | ✅ Done | Gemini via `POST /api/voice/transcribe`, `server/src/voice/gemini-voice-client.ts`, `server/src/controllers/voice.controller.ts` (switched from OpenAI Whisper to Gemini so voice reuses the same `GEMINI_API_KEY` the Agent already needs, instead of a second paid provider) |
| *(beyond BRD scope)* Voice **output** (TTS), natural-sounding | ✅ Done, bonus | Gemini TTS (`gemini-2.5-flash-preview-tts`) via `POST /api/voice/speak` |
